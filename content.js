(function () {
  ...
  async function init() {
    try {
      ...
      console.error("[Glancit] 초기화 에러:", err);
    }
  }
  ...
  function setupSelectionListener() {
    document.addEventListener("mouseup", (e) => {
      ...
      // 마우스 좌표 저장 (말풍선 위치 Fallback용)
      lastMouseCoords = { x: e.clientX, y: e.clientY };

      // 이미 버블이 있다면 제거
      removeBubble();

      // 트리플 클릭의 경우 브라우저 텍스트 선택이 지연되므로 타임아웃 딜레이 연장 (특히 대화 마지막 부분 대응)
      const delay = e.detail >= 3 ? 300 : 10;
  ...
  function showBubble(text, range) {
    ...
    // 구글 챗 등에서 멀티라인 선택 시 bounding rect가 음수(-)이거나 화면 너비를 초과하는 거대한 값이 될 수 있음
    if (!rect || rect.left < 0 || rect.width > window.innerWidth || (rect.width === 0 && rect.height === 0)) {
      const rects = range.getClientRects();
      for (let r of rects) {
        if (r.left >= 0 && r.width > 0 && r.width < window.innerWidth) {
          rect = r;
          break;
        }
      }
    ...
    let x = 0,
      y = 0;
    if (rect && rect.left >= 0 && rect.left < window.innerWidth && rect.width > 0 && rect.width < window.innerWidth) {
      x = rect.left + window.scrollX;
      y = rect.top + window.scrollY;
    } else {
      x = (lastMouseCoords.x || window.innerWidth / 2) + window.scrollX;
      y = (lastMouseCoords.y || window.innerHeight / 2) + window.scrollY;
    }
  ...
  function insertTranslation(range, translation, engineInfo) {
    try {
      ...
      console.error("[Glancit] 결과 삽입 실패:", err);
    }
  }
